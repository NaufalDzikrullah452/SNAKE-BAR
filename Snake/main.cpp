...
#include <string>
#include <ctime>
//#include <SDL_mixer.h> // SDL for audio
...
void renderScore(SDL_Renderer* renderer, int tailLength, int scale, int wScale) {
	...
	TTF_Font* font = TTF_OpenFont((char*)"arial.ttf", 14);
	if (font == NULL) {
...
int main(int argc, char* argv[]) {
	...
	window = SDL_CreateWindow("Snake Bar", SDL_WINDOWPOS_CENTERED, SDL_WINDOWPOS_CENTERED, scale*wScale+1, scale*wScale+1, SDL_WINDOW_RESIZABLE);
	renderer = SDL_CreateRenderer(window, -1, SDL_RENDERER_ACCELERATED);

	float time = SDL_GetTicks() / 120;

	// Main game loop, this constantly runs and keeps everything updated
	while (true) { 

		float newTime = SDL_GetTicks() / 120; //This value (75) is the speed at which the blocks are updated
